			...
			if($jumper=='yes'){
				...
				$jumper_content.= "<div class='evo_j_container' style='display:none' data-m='{$focused_month_num}' data-y='{$focused_year}'>
					...
					// if jumper offset is set
						...
						for($x=1; $x<6; $x++){
							$__a .= '<a'. ( $start_year == $focused_year?" class='current set'":null ).'>'.$start_year.'</a>';
							$start_year++;
						}
	...
	public function generate_event_data(
		...
		foreach($event_list_array as $event):
			...
			*/
			...
			/** CONSTRUCT the EVENT CARD	 **/		
				...
				// EVENT LOCATION
					if( evo_location_exists($ev_vals) ){
						...
						// location as LON LAT
						$event_location_variables = ((!empty($lonlat))? $lonlat:null ). ' data-location_address="'.$ev_vals['evcal_location'][0].'" ';
						...
						$ev_location =				
							'<em class="evcal_location" '.( (!empty($lonlat))? $lonlat:null ).' data-add_str="'.$ev_vals['evcal_location'][0].'">'.$ev_vals['evcal_location'][0].'</em>';
						// location type
			...
			$event_html_code="<div id='event_{$event_id}' class='".implode(' ', $_eventClasses)."' data-event_id='{$event_id}' {$__scheme_attributes} {$show_limit_styles} {$___styles_a} data-colr='{$event_color}'>{$__scheme_data}
